...
export default async function handler(req, res) {
  ...
  // GET: Obter configurações (código de acesso estritamente mascarado)
  if (req.method === 'GET') {
  ...
  // POST: Atualizar configurações (Apenas administradores autenticados)
  if (req.method === 'POST') {
    try {
      const authHeader = req.headers?.authorization || req.headers?.['x-admin-token'] || ''
      const token = authHeader.replace(/^Bearer\s+/i, '').trim()
      const isAuthorized = verifyAdminToken(token) || (token && token === process.env.ADMIN_PASSWORD)

      if (!isAuthorized && process.env.NODE_ENV === 'production') {
        res.statusCode = 401
        res.end(JSON.stringify({
          success: false,
          error: 'Acesso não autorizado. Apenas administradores autenticados podem alterar credenciais dos Correios.'
        }))
        return
      }
