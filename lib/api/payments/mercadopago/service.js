...
export function verifyWebhookSignature({ signatureHeader, xSignature, requestId, xRequestId, dataId, secret }) {
  ...
  if (!secret) {
    if (process.env.NODE_ENV === 'production' && !process.env.ALLOW_INSECURE_WEBHOOKS) {
      console.error('Mercado Pago Webhook: MERCADO_PAGO_WEBHOOK_SECRET ausente em produção! Rejeitando por segurança.')
      return { valid: false, reason: 'SECRET_REQUIRED_IN_PRODUCTION' }
    }
    // Em desenvolvimento ou homologação local sem secret configurado
