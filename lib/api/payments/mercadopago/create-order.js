...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 4. Se o Access Token estiver configurado, cria a Order no Mercado Pago via Orders API
    if (config.isConfigured) {
      ...
      // Grava o pedido no Supabase SOMENTE após a criação bem-sucedida da ordem/preferência no Mercado Pago
      if (supabase) {
        const orderPayload = {
          id: safeOrder.id,
          customer_name: safeOrder.customerName || cliente.nome || cliente.name || 'Cliente',
          customer_email: safeOrder.customerEmail || cliente.email || '',
          customer_cpf: safeOrder.customerCpf || cliente.cpf || '',
          customer_phone: safeOrder.customerPhone || cliente.telefone || cliente.phone || '',
          customer_address: safeOrder.customerAddress || `${cliente.endereco || ''}, ${cliente.numero || ''}`.replace(/^,\s*$/, ''),
        ...
        try {
          ...
          console.warn('Erro ao salvar order no Supabase após criação no Mercado Pago:', err.message)
        }
