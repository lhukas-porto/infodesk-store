// Serviço de pagamento Itaú (simulação)
// Em produção, integrar com API Itaú Shop / Rede
...
export function gerarBoleto(pedido) {
  ...
  const boleto = {
    ...
    beneficiario: 'INFODESK COMÉRCIO DE INFORMÁTICA LTDA',
...
export function gerarBoletoPDF(boleto) {
  ...
  doc.setFontSize(16)
  doc.text(boleto.codigoBanco, 180, 45, { align: 'right' })
...
export function gerarLinkPagamento(pedido) {
  return {
    tipo: 'link_itau',
    url: `https://shopline.itau.com.br/checkout/${Date.now()}`,
    valor: pedido.total,
    valorFormatado: `R$ ${pedido.total.toFixed(2).replace('.', ',')}`,
    expiracao: '24 horas',
